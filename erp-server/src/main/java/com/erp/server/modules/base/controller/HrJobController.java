...
import com.erp.server.modules.base.service.HrJobService;
import lombok.RequiredArgsConstructor;
import org.springframework.web.bind.annotation.*;
@RestController
@RequestMapping("/base/hr-job")
@RequiredArgsConstructor
public class HrJobController {
    private final HrJobService service;
    @GetMapping("/list") public Result<PageResult<HrJobDto>> list(@ModelAttribute HrJobQueryDto q) { return Result.success(service.pageDto(q)); }
    @GetMapping("/{id}") public Result<HrJobDto> get(@PathVariable Long id) { return Result.success(service.getDtoById(id)); }
    @PostMapping public Result<Boolean> save(@RequestBody HrJobDto d) { return Result.success(service.saveDto(d)); }
    @PutMapping("/{id}") public Result<Boolean> update(@PathVariable Long id, @RequestBody HrJobDto d) { return Result.success(service.updateDto(id, d)); }
